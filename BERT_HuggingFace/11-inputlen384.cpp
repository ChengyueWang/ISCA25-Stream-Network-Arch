...
int main() {
    ...
    load_success = load_data("10-gold_384/0-attention.self.query-input.txt", data_input, 384*1024); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/0-attention.self.query-weight.txt", data_weight, 1024*1024); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/0-attention.self.query-bias.txt", data_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/0-attention.self.query-output.txt", gold_val, 384*1024); if (load_success == -1) return -1;
    ...
    print_matrix_to_file("07-cpp_out/0-attention.self.query-output.txt", data_query, 384, 1024);
    ...
    print_matrix_to_file("07-cpp_out/2-attention.self.value-output.txt", data_value, 384, 1024);
    ...
    for (int h=0; h<16; h++){
        ...
        print_matrix_to_file("07-cpp_out/2-attention.output.qkv.txt", qkv, 384, 64);
    ...
    load_success = load_data("10-gold_384/3-attention.output.dense-input.txt", gold_val, 384*1024); if (load_success == -1) return -1;
    print_matrix_to_file("07-cpp_out/3-attention.output.dense-input.txt", self_attention, 384, 1024);
    valid(self_attention, gold_val, 384*1024);


    load_success = load_data("10-gold_384/3-attention.output.dense-weight.txt", data_weight, 1024*1024); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/3-attention.output.dense-bias.txt", data_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/3-attention.output.dense-output.txt", gold_val, 384*1024); if (load_success == -1) return -1;
    calculate_matrix_with_bias (self_attention, data_weight, data_bias, dense1, 384, 1024, 1024);
    print_matrix_to_file("07-cpp_out/3-attention.output.dense-output.txt", dense1, 384, 1024);
    valid(dense1, gold_val, 384*1024);


    matrix_add (dense1, data_input, 384, 1024);
    load_success = load_data("10-gold_384/4-attention.output.LayerNorm-output.txt", gold_val, 384*1024); if (load_success == -1) return -1;
    layer_normalization_no_weightbias(dense1, norm1, 384, 1024);
    print_matrix_to_file("07-cpp_out/4-attention.output.LayerNorm-output.txt", norm1, 384, 1024);
    valid(norm1, gold_val, 384*1024);

    printMatrixToFile(norm1, 384, 1024, "10-gold_384/4-fused_dense1_norm1_output.txt");


    float* norm1_weight = (float*)malloc(1024 * 4);
    float* norm1_bias = (float*)malloc(1024 * 4);
    load_success = load_data("10-gold_384/4-attention.output.LayerNorm-weight.txt", norm1_weight, 1024); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/4-attention.output.LayerNorm-bias.txt", norm1_bias, 1024); if (load_success == -1) return -1;

    float * norm1merged_weight = (float*)malloc(1024*4096 * 4);
    float * norm1merged_bias = (float*)malloc(4096 * 4);
    load_success = load_data("10-gold_384/5-intermediate.dense-weight.txt", data_weight, 1024*4096); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/5-intermediate.dense-bias.txt", data_bias, 4096); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/5-intermediate.dense-output.txt", gold_val, 384*4096); if (load_success == -1) return -1;
    ...
    printMatrixToFile(norm1merged_weight, 1024, 4096, "10-gold_384/5-fused_norm1_dense2_weight.txt");
    printMatrixToFile(norm1merged_bias, 1, 4096, "10-gold_384/5-fused_norm1_dense2_bias.txt");

    // can merge norm1 to dense2, but norm1 need to be used in the norm2 calculation


    print_matrix_to_file("07-cpp_out/5-intermediate.dense-output.txt", dense2, 384, 4096);
    valid(dense2, gold_val, 384*4096);


    cal_gelu (dense2, gelu, 384, 4096);
    load_success = load_data("10-gold_384/6-output.dense-input.txt", gold_val, 384*4096); if (load_success == -1) return -1;
    print_matrix_to_file("07-cpp_out/6-output.dense-input.txt", gelu, 384, 4096);
    valid(gelu, gold_val, 384*4096);

    load_success = load_data("10-gold_384/6-output.dense-weight.txt", data_weight, 4096*1024); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/6-output.dense-bias.txt", data_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/6-output.dense-output.txt", gold_val, 384*1024); if (load_success == -1) return -1;
    
    matrix_add (data_bias, norm1_bias, 1, 1024);

    calculate_matrix_with_bias (gelu, data_weight, data_bias, dense3, 384, 1024, 4096);
    print_matrix_to_file("07-cpp_out/6-output.dense-output.txt", dense3, 384, 1024);
    ...
    load_success = load_data("10-gold_384/7-output.LayerNorm-weight.txt", data_weight, 1024); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/7-output.LayerNorm-bias.txt", data_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("10-gold_384/7-output.LayerNorm-output.txt", gold_val, 384*1024); if (load_success == -1) return -1;
    
    layer_normalization_no_weightbias(dense3, norm2, 384, 1024);
    printMatrixToFile(norm2, 384, 1024, "10-gold_384/7-fused_dense3_norm2.txt");

    layer_normalization (dense3, norm2, data_weight, data_bias, 384, 1024);
    print_matrix_to_file("07-cpp_out/7-output.LayerNorm-output.txt", norm2, 384, 1024);
