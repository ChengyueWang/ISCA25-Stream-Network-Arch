...
void layer_normalization_no_bias(float* matrix, float * matrix_normalized, float* weight, int row, int col){
    ...
    for (int i=0; i<row; i++){
        for (int j=0; j<col; j++){
            matrix_normalized[i*col+j] = matrix_normalized[i*col+j] * weight[j] ;
        }
    }
...
int main() {
    ...
    load_success = load_data("06-python_gold/0-attention.self.query-input.txt", data_input, 512*1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/0-attention.self.query-weight.txt", data_weight, 1024*1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/0-attention.self.query-bias.txt", data_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/0-attention.self.query-output.txt", gold_val, 512*1024); if (load_success == -1) return -1;
    ...
    print_matrix_to_file("07-cpp_out/0-attention.self.query-output.txt", data_query, 512, 1024);
    valid(data_query, gold_val, 512*1024);
    // matrix_scale (data_query, 1.0/8, 512, 1024);


    // load_success = load_data("06-python_gold/1-attention.self.key-input.txt", data_input, 512*1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/1-attention.self.key-weight.txt", data_weight, 1024*1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/1-attention.self.key-bias.txt", data_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/1-attention.self.key-output.txt", gold_val, 512*1024); if (load_success == -1) return -1;
    calculate_matrix_with_bias (data_input, data_weight, data_bias, data_key, 512, 1024, 1024);
    print_matrix_to_file("07-cpp_out/1-attention.self.key-output.txt", data_key, 512, 1024);
    valid(data_key, gold_val, 512*1024);

    // load_success = load_data("06-python_gold/2-attention.self.value-input.txt", data_input, 512*1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/2-attention.self.value-weight.txt", data_weight, 1024*1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/2-attention.self.value-bias.txt", data_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/2-attention.self.value-output.txt", gold_val, 512*1024); if (load_success == -1) return -1;
    calculate_matrix_with_bias (data_input, data_weight, data_bias, data_value, 512, 1024, 1024);
    print_matrix_to_file("07-cpp_out/2-attention.self.value-output.txt", data_value, 512, 1024);
    ...
    for (int h=0; h<16; h++){
        ...
        print_matrix_to_file("07-cpp_out/2-attention.output.qk.txt", qk, 512, 512);
        calculate_softmax(qk, qk_softmax, 512, 512);
        print_matrix_to_file("07-cpp_out/2-attention.output.qk_softmax.txt", qk_softmax, 512, 512);
        calculate_matrix(qk_softmax, v, qkv, 512, 64, 512);
        print_matrix_to_file("07-cpp_out/2-attention.output.qkv.txt", qkv, 512, 64);
    ...
    load_success = load_data("06-python_gold/3-attention.output.dense-input.txt", gold_val, 512*1024); if (load_success == -1) return -1;
    print_matrix_to_file("07-cpp_out/3-attention.output.dense-input.txt", self_attention, 512, 1024);
    valid(self_attention, gold_val, 512*1024);


    load_success = load_data("06-python_gold/3-attention.output.dense-weight.txt", data_weight, 1024*1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/3-attention.output.dense-bias.txt", data_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/3-attention.output.dense-output.txt", gold_val, 512*1024); if (load_success == -1) return -1;
    calculate_matrix_with_bias (self_attention, data_weight, data_bias, dense1, 512, 1024, 1024);
    print_matrix_to_file("07-cpp_out/3-attention.output.dense-output.txt", dense1, 512, 1024);
    valid(dense1, gold_val, 512*1024);

    float * norm1_bias = (float*)malloc(1024*4);
    matrix_add (dense1, data_input, 512, 1024);
    load_success = load_data("06-python_gold/4-attention.output.LayerNorm-weight.txt", data_weight, 1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/4-attention.output.LayerNorm-bias.txt", norm1_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/4-attention.output.LayerNorm-output.txt", gold_val, 512*1024); if (load_success == -1) return -1;
    layer_normalization_no_bias (dense1, norm1, data_weight, 512, 1024);
    print_matrix_to_file("07-cpp_out/4-attention.output.LayerNorm-output.txt", norm1, 512, 1024);
    valid(norm1, gold_val, 512*1024);


    load_success = load_data("06-python_gold/5-intermediate.dense-weight.txt", data_weight, 1024*4096); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/5-intermediate.dense-bias.txt", data_bias, 4096); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/5-intermediate.dense-output.txt", gold_val, 512*4096); if (load_success == -1) return -1;

    matrix_add (data_bias, norm1_bias, 1, 1024);

    calculate_matrix_with_bias (norm1, data_weight, data_bias, dense2, 512, 4096, 1024);
    print_matrix_to_file("07-cpp_out/5-intermediate.dense-output.txt", dense2, 512, 4096);
    valid(dense2, gold_val, 512*4096);

    cal_gelu (dense2, gelu, 512, 4096);
    load_success = load_data("06-python_gold/6-output.dense-input.txt", gold_val, 512*4096); if (load_success == -1) return -1;
    print_matrix_to_file("07-cpp_out/6-output.dense-input.txt", gelu, 512, 4096);
    valid(gelu, gold_val, 512*4096);

    load_success = load_data("06-python_gold/6-output.dense-weight.txt", data_weight, 4096*1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/6-output.dense-bias.txt", data_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/6-output.dense-output.txt", gold_val, 512*1024); if (load_success == -1) return -1;
    calculate_matrix_with_bias (gelu, data_weight, data_bias, dense3, 512, 1024, 4096);
    print_matrix_to_file("07-cpp_out/6-output.dense-output.txt", dense3, 512, 1024);
    valid(dense3, gold_val, 512*1024);

    matrix_add (dense3, norm1, 512, 1024);
    load_success = load_data("06-python_gold/7-output.LayerNorm-weight.txt", data_weight, 1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/7-output.LayerNorm-bias.txt", data_bias, 1024); if (load_success == -1) return -1;
    load_success = load_data("06-python_gold/7-output.LayerNorm-output.txt", gold_val, 512*1024); if (load_success == -1) return -1;
    layer_normalization (dense3, norm2, data_weight, data_bias, 512, 1024);
    print_matrix_to_file("07-cpp_out/7-output.LayerNorm-output.txt", norm2, 512, 1024);
