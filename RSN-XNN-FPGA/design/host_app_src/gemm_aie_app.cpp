...
int main(int argc, char** argv) {
  if (argc == 2 || (argc == 3 && strcmp(argv[2], "RUN_CODE") == 0)) {
    ...
    std::cout << "Usage: " << argv[0] << " <xclbin> [RUN_CODE]" << std::endl;
    return 1;
  }
...
#ifdef SW_EMU_PRINT
  std::string command =
      "rm -f /home/cw4/github/versal-float32/19-gelu-norm-bias/design/pl_src/output/*";
  int result = std::system(command.c_str());
  std::ofstream outFile(
      "/home/cw4/github/versal-float32/19-gelu-norm-bias/design/pl_src/output/00_out_main.txt");
  ...
  std::string hls_obj_str;
  hls_obj_str = "dma_hls:{dma_hls_0}";
  const char* hls_obj;
  hls_obj = hls_obj_str.c_str();
...
#ifdef SW_EMU_PRINT
  python_gold_file =
      "/home/cw4/github/versal-float32/19-gelu-norm-bias/workspace/app_component/host_app_src/"
      "python_gold/";
#else
  ...
  // print_matrix_to_file("/home/cw4/github/versal-float32/19-gelu-norm-bias/design/pl_src/output/01_0-attention.self.query-output.txt",
  // fmap1_query, 512, 1024); valid(fmap1_query, gold_query, 6*512*1024);
  ...
  load_data(python_gold_file + "3-attention.output.dense-input.txt", load_gold_attention,
            512 * 1024);
  ...
  params_attention.num_layer = 96;

  std::cout << "layer fused_dense1_norm1" << std::endl;

  // load_data(python_gold_file + "3-attention.output.dense-input.txt", fmap4_self_attention,
  // 512*1024); replicate_matrix (fmap4_self_attention, 512, 1024, 6);
  ...
  load_data(python_gold_file + "4-fused_dense1_norm1_output.txt", load_gold_fuseddense1norm1,
            512 * 1024);
  ...
  convert_input_weight_gold_to_2AXI(fmap6_norm1, weight5_dense2, load_gold_norm1dense2gelu,
                                    params_dense2);
  ...
  // generate_data_onelayer_with_bias(fmap6_norm1, weight5_dense2, bias5_dense2, fmap7_dense2,
  // load_gold_norm1dense2gelu, params_dense2); generate_data_onelayer_with_bias(fmap0_embedding,
  // weight0_query, bias0_query, fmap1_query, load_gold_query, params_query);
  // generate_data_fusedlayer(fmap1_query, fmap2_key, fmap3_value, fmap4_self_attention,
  // load_gold_attention, params_attention); generate_data_onelayer_norm(
  ...
  // generate_instruction_onelayer_overlap_128_512_A4B1K8(host_instruction + 2, count4B,
  // countInstPkt, params_query); generate_instruction_onelayer(host_instruction + 2, count4B,
  // countInstPkt, params_dense2);
  // generate_instruction_onelayer_overlap_128_512_A4B4K8(host_instruction + 2, count4B,
  // countInstPkt, params_dense2); generate_instruction_fusedlayer_numlayer1(host_instruction + 2 ,
  // count4B, countInstPkt, params_attention);
  // generate_instruction_onelayer_norm_overlap_A4B1K8(host_instruction + 2, count4B, countInstPkt,
  // params_norm1); generate_instruction_onelayer_overlap_128_512_A4B1K8(host_instruction + 2,
  // count4B, countInstPkt, params_query);

  // #ifdef ENABLE_TEST_Q
  // // generate_instruction_onelayer(host_instruction + 2, count4B, countInstPkt, params_query);
  // generate_instruction_onelayer_overlap_128_512_A4B1K8(host_instruction + 2, count4B,
  // countInstPkt, params_query); #endif #ifdef ENABLE_TEST_K
  // // generate_instruction_onelayer(host_instruction + 2, count4B, countInstPkt, params_key);
  // generate_instruction_onelayer_overlap_128_512_A4B1K8(host_instruction + 2, count4B,
  // countInstPkt, params_key); #endif #ifdef ENABLE_TEST_V
  // // generate_instruction_onelayer(host_instruction + 2, count4B, countInstPkt, params_value);
  // generate_instruction_onelayer_overlap_128_512_A4B1K8(host_instruction + 2, count4B,
  // countInstPkt, params_value); #endif #ifdef ENABLE_TEST_ATTENTION
  // generate_instruction_fusedlayer(host_instruction + 2 , count4B, countInstPkt,
  // params_attention); #endif #ifdef ENABLE_TEST_NORM1
  // // generate_instruction_onelayer_norm(host_instruction + 2, count4B, countInstPkt,
  // params_norm1); generate_instruction_onelayer_norm_overlap_A4B1K8(host_instruction + 2, count4B,
  // countInstPkt, params_norm1); #endif #ifdef ENABLE_TEST_GELU
  // // generate_instruction_onelayer(host_instruction + 2, count4B, countInstPkt, params_dense2);
  // generate_instruction_onelayer_first2load_A4B4K8(host_instruction + 2, count4B, countInstPkt,
  // params_dense2); #endif #ifdef ENABLE_TEST_NORM2
  // // generate_instruction_onelayer_norm(host_instruction + 2, count4B, countInstPkt,
  // params_norm2); generate_instruction_onelayer_norm_overlap_A4B1K32(host_instruction + 2,
  // count4B, countInstPkt, params_norm2); #endif

#if defined(ENABLE_TEST_Q) && defined(ENABLE_TEST_K) && defined(ENABLE_TEST_V)
  generate_instruction_query_key_value(host_instruction + 2, count4B, countInstPkt, params_query,
  ...
  generate_instruction_norm1_gelu_norm2(host_instruction + 2, count4B, countInstPkt, params_norm1,
                                        params_dense2, params_norm2);
#endif
  ...
  // xrtGraphEnd(gemm_aie_gr, 0); // if use xrtGraphEnd(gemm_aie_gr, 16) or xrtGraphWait the sw emu
  // report undefined warming
  ...
  std::string file_name =
      "/home/cw4/github/versal-float32/19-gelu-norm-bias/design/pl_src/output/03_";
  print_matrix_to_file(file_name + "fmap1_query_2AXI.txt", fmap1_query,
                       params_query.getMatrixDim_A(), params_query.getMatrixDim_B());
#endif

#ifdef ENABLE_TEST_Q
  std::cout << "Compare query" << std::endl;
  valid(fmap1_query, load_gold_query,
        params_query.getMatrixDim_A() * params_query.getMatrixDim_B());
#endif

#ifdef ENABLE_TEST_K
  std::cout << "Compare key" << std::endl;
  valid(fmap2_key, load_gold_key, 6 * 512 * 1024);
#endif

#ifdef ENABLE_TEST_V
  std::cout << "Compare value" << std::endl;
  valid(fmap3_value, load_gold_value, 6 * 512 * 1024);
#endif
  ...
  valid(fmap6_norm1, load_gold_fuseddense1norm1, 6 * 512 * 1024);
#endif

#ifdef ENABLE_TEST_GELU
  std::cout << "Compare fused norm1 dense2 gelu" << std::endl;
  valid(fmap7_dense2, load_gold_norm1dense2gelu, 6 * 512 * 4096);
#endif

#ifdef ENABLE_TEST_NORM2
  std::cout << "Compare fused dense3 norm2" << std::endl;
  valid(fmap10_norm2, load_gold_fuseddense3norm2, 6 * 512 * 1024);
#endif
