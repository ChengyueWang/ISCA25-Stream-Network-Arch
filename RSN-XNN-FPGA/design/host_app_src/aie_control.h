...
void write_opcode_mem(uint32_t *array, int core_id, int dima_id, int dimb_id, int casc_id,
                      int offset, uint32_t value) {
  array[core_id * 4 * 4 * 4 * 512 + dima_id * 4 * 4 * 512 + dimb_id * 4 * 512 + casc_id * 512 +
        offset] = value;
}

uint32_t read_opcode_mem(uint32_t *array, int core_id, int dima_id, int dimb_id, int casc_id,
                         int offset) {
  return array[core_id * 4 * 4 * 4 * 512 + dima_id * 4 * 4 * 512 + dimb_id * 4 * 512 +
               casc_id * 512 + offset];
}
...
void print_opcode_mem_array(uint32_t *array) {
  ...
  std::ofstream outFile(
      "/home/cw4/github/versal-float32/19-gelu-norm-bias/design/pl_src/output/"
      "01_opcode_mem_array.txt",
      std::ios_base::app);
  if (!outFile.is_open()) {
    ...
    for (int dima_id = 0; dima_id < 4; dima_id++) {
      for (int dimb_id = 0; dimb_id < 4; dimb_id++) {
        for (int csc = 0; csc < 4; csc++) {
          outFile << "core_id = " << core_id << " dima_id = " << dima_id << " dimb_id = " << dimb_id
                  << " csc = " << csc << std::endl;
          ...
          for (int i = 2; i < 16; i++) {
            ...
            outFile << " compute_tile_access_A = " << compute_tile_access_A
                    << " compute_tile_access_B = " << compute_tile_access_B;
            outFile << " enable_load_pre_norm_weight = " << enable_load_pre_norm_weight
                    << " enable_bias = " << enable_bias;
            outFile << " dimb_is_32 = " << dimb_is_32 << " dimk_is_32 = " << dimk_is_32;
            outFile << " enable_accum_kiter = " << enable_accum_kiter
                    << " enable_muladd_pre_layer = " << enable_muladd_pre_layer;
...
void generate_aie_rtp_opcode_mem(xrtGraphHandle gemm_aie_gr) {
  ...
  for (int core_id = 4; core_id < 6; core_id++) {
    for (int dima_id = 0; dima_id < 4; dima_id++) {
      for (int dimb_id = 0; dimb_id < 4; dimb_id++) {
        for (int csc = 0; csc < 4; csc++) {
#ifdef ENABLE_TEST_ATTENTION
  ...
  for (int core_id = 0; core_id < 6; core_id++) {
    for (int dima_id = 0; dima_id < 4; dima_id++) {
      for (int dimb_id = 0; dimb_id < 4; dimb_id++) {
        ...
        const char *opcode_ptr0 =
            reinterpret_cast<const char *>(opcode_mem + core_id * 4 * 4 * 4 * 512 +
                                           dima_id * 4 * 4 * 512 + dimb_id * 4 * 512 + 0 * 512);
        rtp_opcode = "G.FG[" + to_string(core_id) + "][" + to_string(dima_id) + "][" +
                     to_string(dimb_id) + "].k0.in[2]";
        rtp_opcode_obj = rtp_opcode.c_str();
        xrtGraphUpdateRTP(gemm_aie_gr, rtp_opcode_obj, opcode_ptr0, 512 * sizeof(uint32_t));

        const char *opcode_ptr1 =
            reinterpret_cast<const char *>(opcode_mem + core_id * 4 * 4 * 4 * 512 +
                                           dima_id * 4 * 4 * 512 + dimb_id * 4 * 512 + 1 * 512);
        rtp_opcode = "G.FG[" + to_string(core_id) + "][" + to_string(dima_id) + "][" +
                     to_string(dimb_id) + "].k1.in[3]";
        rtp_opcode_obj = rtp_opcode.c_str();
        xrtGraphUpdateRTP(gemm_aie_gr, rtp_opcode_obj, opcode_ptr1, 512 * sizeof(uint32_t));

        const char *opcode_ptr2 =
            reinterpret_cast<const char *>(opcode_mem + core_id * 4 * 4 * 4 * 512 +
                                           dima_id * 4 * 4 * 512 + dimb_id * 4 * 512 + 2 * 512);
        rtp_opcode = "G.FG[" + to_string(core_id) + "][" + to_string(dima_id) + "][" +
                     to_string(dimb_id) + "].k2.in[3]";
        rtp_opcode_obj = rtp_opcode.c_str();
        xrtGraphUpdateRTP(gemm_aie_gr, rtp_opcode_obj, opcode_ptr2, 512 * sizeof(uint32_t));

        const char *opcode_ptr3 =
            reinterpret_cast<const char *>(opcode_mem + core_id * 4 * 4 * 4 * 512 +
                                           dima_id * 4 * 4 * 512 + dimb_id * 4 * 512 + 3 * 512);
        rtp_opcode = "G.FG[" + to_string(core_id) + "][" + to_string(dima_id) + "][" +
                     to_string(dimb_id) + "].k3.in[3]";
        rtp_opcode_obj = rtp_opcode.c_str();
