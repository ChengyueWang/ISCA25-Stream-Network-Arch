...
void generate_instruction_Klarger16_steady_ddr(int loadA_a, int loadA_b, int storeC_a, int storeC_b,
  ...
  inst_ddr.inst_val.start_address =
      storeC_address + (4 * b_iter + 1) * 128 * 512 / 16 + 128 * 1024 * b_iter / 16;
  inst_ddr.inst_val.address_offset = 0;
  inst_ddr.inst_val.chunck_size = 128 * 512;
  inst_ddr.inst_val.stride_offset = 128 * 1024 * b_iter / 16;
  inst_ddr.inst_val.chunk_count = 1;
  ...
  inst_ddr.inst_val.start_address =
      storeC_address + (8 * b_iter) * 128 * 512 / 16 + 128 * 1024 * b_iter / 16;
  inst_ddr.inst_val.address_offset = 0;
  inst_ddr.inst_val.chunck_size = 128 * 512;
  inst_ddr.inst_val.stride_offset = 128 * 1024 * b_iter / 16;
  inst_ddr.inst_val.chunk_count = 1;
  ...
  inst_ddr.inst_val.start_address =
      storeC_address + (8 * b_iter + 1) * 128 * 512 / 16 + 128 * 1024 * b_iter / 16;
  inst_ddr.inst_val.address_offset = 0;
  inst_ddr.inst_val.chunck_size = 128 * 512;
  inst_ddr.inst_val.stride_offset = 128 * 1024 * b_iter / 16;
  inst_ddr.inst_val.chunk_count = 1;
...
void generate_load_weight_previous_norm(uint32_t *inst_sequence, uint32_t &count4B,
                                        uint32_t &countInstPkt, ParamsOneLayer params) {
  union_inst_header inst_header;
  union_inst_ddr inst_ddr;
  union_inst_load_B_dram inst_loadB_dram;
  ...
  inst_mesh_B.inst_val.size =
      1 * 1 * b_iter * 1 * 1 * 32 * 32 / 2;  // 1 * 1 * b_iter*1*1*16*8/2 ; // when b_iter == 1 ,
                                             // 1024 total bias, 1 AIE only got 128 bias
...
void generate_instruction_onelayer_cores_meshes_add_prev_layer(uint32_t *inst_sequence,
                                                               uint32_t &count4B,
...
void generate_instruction_onelayer_cores_meshes_add_prev_layer_prolog(uint32_t *inst_sequence,
                                                                      uint32_t &count4B,
                                                                      uint32_t &countInstPkt,
                                                                      ParamsOneLayer params) {
  uint32_t a_iter = 1;
...
void generate_instruction_onelayer_cores_meshes_add_prev_layer_epilog(uint32_t *inst_sequence,
                                                                      uint32_t &count4B,
                                                                      uint32_t &countInstPkt,
                                                                      ParamsOneLayer params) {
  uint32_t a_iter = 1;
...
void generate_instruction_onelayer_cores_meshes_add_prev_layer_steady(uint32_t *inst_sequence,
                                                                      uint32_t &count4B,
                                                                      uint32_t &countInstPkt,
                                                                      ParamsOneLayer params) {
  uint32_t a_iter = 1;
...
void generate_instruction_load_prev_layer(int a, int b, uint32_t *inst_sequence, uint32_t &count4B,
                                          uint32_t &countInstPkt, ParamsOneLayer params) {
  uint32_t a_iter = params.a_iter;
  uint32_t b_iter = params.b_iter;
  uint32_t k_iter = params.k_iter;
  ...
  loadA_address = params.ddr_offset_512b_pre_layer + a * 768 * 1024 / 16;
  inst_header.inst_val.opcode = OPCODE_DDR_DRAM;
  inst_header.inst_val.mask = false;
  inst_header.inst_val.is_last_mOP = false;
  ...
  inst_ddr.inst_val.start_address =
      loadA_address + 0 / 16 +
      128 * 128 * 4 / 16;  // divided by 16 because 1 address hold 16 32float (512bit)
  inst_ddr.inst_val.address_offset = 128 * 128 / 16;
  inst_ddr.inst_val.chunck_size = 128 * 128;  // 2 tiles, each tile 128x64
  ...
  loadA_address = params.ddr_offset_512b_pre_layer + a * 768 * 1024 / 16 + 128 * 1024 / 16;
  inst_header.inst_val.opcode = OPCODE_DDR_DRAM;
  inst_header.inst_val.mask = false;
  inst_header.inst_val.is_last_mOP = false;
  inst_header.inst_val.mop_buffer_window_size = 6;
...
void generate_instruction_onelayer_norm(uint32_t *inst_sequence, uint32_t &count4B,
                                        uint32_t &countInstPkt, ParamsOneLayer params) {
  uint32_t a_iter = params.a_iter;
  uint32_t b_iter = params.b_iter;
  uint32_t k_iter = params.k_iter;
  ...
  a = 0;
  b = 0;
  generate_instruction_onelayer_cores_meshes_add_prev_layer(inst_sequence, count4B, countInstPkt,
                                                            params);
...
void generate_instruction_onelayer_norm_overlap_A4B1K8(uint32_t *inst_sequence, uint32_t &count4B,
                                                       uint32_t &countInstPkt,
                                                       ParamsOneLayer params) {
  uint32_t a_iter = params.a_iter;
  uint32_t b_iter = params.b_iter;
  ...
  a = 0;
  b = 0;
  generate_instruction_onelayer_cores_meshes_add_prev_layer_prolog(inst_sequence, count4B,
                                                                   countInstPkt, params);
  generate_instruction_load_prev_layer(a, b, inst_sequence, count4B, countInstPkt, params);
  generate_instruction_loadB(a, b, inst_sequence, count4B, countInstPkt, params);
  generate_instruction_loadA(a, b, 0, k_iter, inst_sequence, count4B, countInstPkt, params);
  ...
  a = 1;
  b = 0;
  generate_instruction_onelayer_cores_meshes_add_prev_layer_steady(inst_sequence, count4B,
                                                                   countInstPkt, params);
  generate_instruction_load_prev_layer(a, b, inst_sequence, count4B, countInstPkt, params);
  generate_instruction_loadB(a, b, inst_sequence, count4B, countInstPkt, params);
  // generate_instruction_loadA(a, b, 0, k_iter, inst_sequence, count4B, countInstPkt, params);
  ...
  a = 3;
  b = 0;
  generate_instruction_onelayer_cores_meshes_add_prev_layer_epilog(inst_sequence, count4B,
                                                                   countInstPkt, params);
  generate_instruction_load_prev_layer(a, b, inst_sequence, count4B, countInstPkt, params);
  generate_instruction_loadB(a, b, inst_sequence, count4B, countInstPkt, params);
  // generate_instruction_loadA(a, b, 0, k_iter, inst_sequence, count4B, countInstPkt, params);
...
void generate_instruction_onelayer_norm_overlap_A4B1K32(uint32_t *inst_sequence, uint32_t &count4B,
                                                        uint32_t &countInstPkt,
                                                        ParamsOneLayer params) {
  uint32_t a_iter = params.a_iter;
  uint32_t b_iter = params.b_iter;
  ...
  storeC_a = 0;
  storeC_b = 0;
  generate_instruction_Klarger16_steady_ddr(loadA_a, loadA_b, storeC_a, storeC_b, inst_sequence,
