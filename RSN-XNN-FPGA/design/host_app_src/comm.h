...
static std::vector<char> load_xclbin(xrtDeviceHandle device, const std::string& fnm) {
  if (fnm.empty()) throw std::runtime_error("No xclbin speified");
...
class ParamsOneLayer {
 ...
 public:
  ...
  void set_ddr_size_for_inA(size_t size) { this->ddr_size_for_inA = size; }
  void set_ddr_size_for_outC(size_t size) { this->ddr_size_for_outC = size; }

  void set_lpddr_size_for_inB(size_t size) { this->lpddr_size_for_inB = size; }
  ...
  uint32_t getAIE_round() {
    return k_iter * a_iter * b_iter * compute_tile_access_A * compute_tile_access_B;
  }
  uint32_t getAIE_k_iter() { return 1; }
  double getTotalOps() { return 2 * pl_tile_a * a_iter * pl_tile_b * b_iter * pl_tile_k * k_iter; }
  double calThroughput(double time) {
    return ((2 * getMatrixDim_A() * getMatrixDim_B() / 1000) * getMatrixDim_K()) / time;
  }

  size_t getDDR_outC_offset_in_size() { return ddr_size_for_inA; }

  size_t getDDR_Bytes() { return 4 * ddr_size_for_inA + 4 * ddr_size_for_outC; }
  size_t getLPDDR_Bytes() { return 4 * lpddr_size_for_inB; }

  void print() {
    std::cout << "A_ITER = " << a_iter << " B_ITER = " << b_iter << " K_ITER = " << k_iter
              << std::endl;
    std::cout << "MATRIX_A = " << getMatrixDim_A() << " MATRIX_B = " << getMatrixDim_B()
              << " MATRIX_K = " << getMatrixDim_K() << std::endl;
...
class ParamsFusedLayer {
 ...
 public:
  ...
  void set_num_layer(uint32_t num_layer) { this->num_layer = num_layer; }
  void set_ddr_size_for_L1_inA(size_t size) { this->ddr_size_for_L1_inA = size; }
  void set_ddr_size_for_L1_inB(size_t size) { this->ddr_size_for_L1_inB = size; }
  void set_ddr_size_for_L2_inB(size_t size) { this->ddr_size_for_L2_inB = size; }
  void set_ddr_size_for_L2_outC(size_t size) { this->ddr_size_for_L2_outC = size; }
  ...
  size_t getDDR_L1_inA_offset_in_size() { return 0; }
  size_t getDDR_L1_inB_offset_in_size() { return ddr_size_for_L1_inA; }
  size_t getDDR_L2_inB_offset_in_size() { return ddr_size_for_L1_inA + ddr_size_for_L1_inB; }
  ...
  double calThroughput(double time) {
    return ((512 * 64 * 512 + 512 * 512 * 64) / 1000) * num_layer * 2 / time;
  }

  size_t getDDR_Bytes() {
    return 4 * ddr_size_for_L1_inA + 4 * ddr_size_for_L1_inB + 4 * ddr_size_for_L2_inB +
           4 * ddr_size_for_L2_outC;
  }
  size_t getLPDDR_Bytes() { return 1024; }  // size 0 will cause fail in allocatation

  void print() {
    std::cout << "NUM_LAYER = " << num_layer << std::endl;
    std::cout << "MATRIX_L1_A = " << getMatrixDim_L1_A() << " MATRIX_L1_K = " << getMatrixDim_L1_K()
              << " MATRIX_L1_B = " << getMatrixDim_L1_B() << std::endl;
    std::cout << "AIE_ITER_L1 = " << getAIEIterL1() << " AIE_ITER_L2 = " << getAIEIterL2()
              << std::endl;
  }
};
