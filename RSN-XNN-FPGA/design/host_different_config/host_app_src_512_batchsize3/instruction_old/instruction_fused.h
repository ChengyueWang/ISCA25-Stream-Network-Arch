...
void generate_instruction_fusedlayer_numlayer3 ( uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsFusedLayer & params) {
    ...
    uint32_t ddr_addr_L1_inA  = params.getDDR_L1_inA_offset_in_size()/16;
    uint32_t ddr_addr_L1_inB  = params.getDDR_L1_inB_offset_in_size()/16;
    uint32_t ddr_addr_L2_inB  = params.getDDR_L2_inB_offset_in_size()/16;
    uint32_t ddr_addr_L2_outC = params.getDDR_L2_outC_offset_in_size()/16;
    ...
    inst_ddr.inst_val.stride_offset = num_layer * 128*64/16; // @@@
    ...
    inst_ddr.inst_val.start_address = ddr_addr_L1_inA + num_layer * 2*128*64/16 ;  // @@@
    inst_ddr.inst_val.address_offset = 64*128 / 16; // @@@
    inst_ddr.inst_val.chunck_size = 128*64; 
    inst_ddr.inst_val.stride_offset = num_layer *  128*64/16; // @@@
    inst_ddr.inst_val.chunk_count = 2; 
    ...
    inst_ddr.inst_val.load_to_memcoreC5 = false;
    for (int i = 0; i < CNT4B_DDR ; i++) { inst_sequence[count4B++] = inst_ddr.raw_32b[i];    }
    inst_ddr.inst_val.start_address = ddr_addr_L1_inB + num_layer * 2*128*64/16 ;  // @@@
    inst_ddr.inst_val.address_offset = 64*128 / 16; // @@@
    inst_ddr.inst_val.chunck_size = 128*64; 
    inst_ddr.inst_val.stride_offset = num_layer * 128*64/16;  // @@@
    inst_ddr.inst_val.chunk_count = 2; 
    ...
    inst_header.inst_val.repeat_num = num_layer - 2 ;
    inst_sequence[count4B++] = inst_header.raw_32b;
    printHeaderInst(inst_header.inst_val, countInstPkt);
    countInstPkt ++;
    inst_ddr.inst_val.start_address = ddr_addr_L1_inA; 
    ...
    inst_ddr.inst_val.store_from_memcoreC5 = false;
    for (int i = 0; i < CNT4B_DDR ; i++) {inst_sequence[count4B++] = inst_ddr.raw_32b[i];    }
    inst_ddr.inst_val.start_address = ddr_addr_L2_outC + num_layer*2*128*64/16; 
    inst_ddr.inst_val.address_offset = 64*128/16; 
    inst_ddr.inst_val.chunck_size = 128*64;
    inst_ddr.inst_val.stride_offset = num_layer*128*64/16;
    inst_ddr.inst_val.chunk_count = 2; 
    ...
    ddr_addr_L2_outC +=  (64*128/16) * (num_layer-2) ;
    inst_header.inst_val.opcode = OPCODE_DDR_DRAM;
    inst_header.inst_val.mask = false;
    inst_header.inst_val.is_last_mOP = false;
...
void generate_instruction_fusedlayer ( uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsFusedLayer & params) {
    ...
    if(num_layer == 1){
        generate_instruction_fusedlayer_numlayer1(inst_sequence, count4B, countInstPkt, params);
    } else if (num_layer >= 3 ) {
        generate_instruction_fusedlayer_numlayer3(inst_sequence, count4B, countInstPkt, params);
    } else {
        assert(0);
    }
