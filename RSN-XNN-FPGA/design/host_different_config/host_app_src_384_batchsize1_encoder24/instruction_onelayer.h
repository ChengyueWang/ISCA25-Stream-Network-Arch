...
void generate_instruction_onelayer_coresAB_meshesAB ( uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsOneLayer params){
...
void generate_instruction_onelayer_cores_meshes ( uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsOneLayer params){
   ...
   inst_header.inst_val.opcode = OPCODE_MEMCORE_B;
    ...
    printHeaderInst(inst_header.inst_val, countInstPkt);
    countInstPkt ++;
    inst_mesh_A.inst_val.size = k_iter * a_iter * b_iter*2*4*32*32/4 ;
    inst_mesh_A.inst_val.memcoreA0_to_computecore0 = true;
    inst_mesh_A.inst_val.memcoreA0_to_computecore1 = true; 
    inst_mesh_A.inst_val.memcoreA1_to_computecore2 = true;
    inst_mesh_A.inst_val.memcoreA1_to_computecore3 = true;
    ...
    printHeaderInst(inst_header.inst_val, countInstPkt);
    countInstPkt ++;
    inst_mesh_B.inst_val.size = k_iter * a_iter * b_iter*2*4*32*32/4 ;
    inst_mesh_B.inst_val.memcore0_to_computecore0 = true;
    inst_mesh_B.inst_val.memcore0_to_computecore2 = true;
    inst_mesh_B.inst_val.memcore0_to_computecore4 = true;
    inst_mesh_B.inst_val.memcore1_to_computecore1 = true;
...
void generate_bias(uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsOneLayer params){
    ...
    printHeaderInst(inst_header.inst_val, countInstPkt);
    countInstPkt ++;
    inst_mesh_B.inst_val.size =  1 * 1 * 1*1*1*32*32/2 ; // 1 * 1 * b_iter*1*1*16*8/2 ; // when b_iter == 1 , 1024 total bias, 1 AIE only got 128 bias
    inst_mesh_B.inst_val.memcore0_to_computecore0 = true;
    inst_mesh_B.inst_val.memcore0_to_computecore2 = true;
    inst_mesh_B.inst_val.memcore0_to_computecore4 = true;
    inst_mesh_B.inst_val.memcore1_to_computecore1 = true;
...
void generate_instruction_loadA (int a, int b, int offset, int repeat_num, uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsOneLayer params){
    ...
    loadA_address += a * 768/2 * 128 * k_iter / 16 + offset;
    ...
    inst_ddr.inst_val.start_address = loadA_address + k_iter*(256/2)*128 / 16;
    inst_ddr.inst_val.address_offset = 128*128 / 16;
    ...
    inst_ddr.inst_val.store_from_memcoreC5 = false;
    for (int i = 0; i < CNT4B_DDR ; i++) { inst_sequence[count4B++] = inst_ddr.raw_32b[i];}
    inst_ddr.inst_val.start_address = loadA_address + k_iter*(256/2)*128 * 2 / 16;
    inst_ddr.inst_val.address_offset = 128*128 / 16;
    inst_ddr.inst_val.chunck_size = 128*128; // 2 tiles, each 128x64
    inst_ddr.inst_val.stride_offset = 128*128*k_iter /16;
    inst_ddr.inst_val.chunk_count = 1; 
...
void generate_instruction_storeC(int a, int b, uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsOneLayer params){
    ...
    storeC_address += (a * 768/2 * 1024 * b_iter + b * 128 * 1024) / 16;       
    inst_header.inst_val.opcode = OPCODE_DDR_DRAM;
    inst_header.inst_val.mask = false;
    inst_header.inst_val.is_last_mOP = false;
    ...
    inst_ddr.inst_val.store_from_memcoreC5 = false;
    for (int i = 0; i < CNT4B_DDR ; i++) { inst_sequence[count4B++] = inst_ddr.raw_32b[i]; }
    inst_ddr.inst_val.start_address = storeC_address + (4*b_iter/2)*128*512/16;
    inst_ddr.inst_val.address_offset = 0; 
    inst_ddr.inst_val.chunck_size = 128*512;
    inst_ddr.inst_val.stride_offset = 128*1024*b_iter/16;
    inst_ddr.inst_val.chunk_count = 1; 
    ...
    inst_ddr.inst_val.store_from_memcoreC5 = false;
    for (int i = 0; i < CNT4B_DDR ; i++) { inst_sequence[count4B++] = inst_ddr.raw_32b[i]; }
    inst_ddr.inst_val.start_address = storeC_address + (4*b_iter/2+1)*128*512/16;
    inst_ddr.inst_val.address_offset = 0; 
    inst_ddr.inst_val.chunck_size = 128*512;
    inst_ddr.inst_val.stride_offset = 128*1024*b_iter/16;
    inst_ddr.inst_val.chunk_count = 1; 
    ...
    inst_ddr.inst_val.store_from_memcoreC5 = false;
    for (int i = 0; i < CNT4B_DDR ; i++) { inst_sequence[count4B++] = inst_ddr.raw_32b[i]; }
    inst_ddr.inst_val.start_address = storeC_address + (8*b_iter/2)*128*512/16;
    inst_ddr.inst_val.address_offset = 0; 
    inst_ddr.inst_val.chunck_size = 128*512;
    inst_ddr.inst_val.stride_offset = 128*1024*b_iter/16;
    inst_ddr.inst_val.chunk_count = 1; 
    ...
    inst_ddr.inst_val.store_from_memcoreC5 = false;
    for (int i = 0; i < CNT4B_DDR ; i++) { inst_sequence[count4B++] = inst_ddr.raw_32b[i]; }
    inst_ddr.inst_val.start_address = storeC_address + (8*b_iter/2+1)*128*512/16;
    inst_ddr.inst_val.address_offset = 0; 
    inst_ddr.inst_val.chunck_size = 128*512;
    inst_ddr.inst_val.stride_offset = 128*1024*b_iter/16;
    inst_ddr.inst_val.chunk_count = 1; 
...
void generate_instruction_K8_steady_colapsetwolayer_storeCloadA (int loadA_a, int loadA_b, int storeC_a, int storeC_b, uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsOneLayer params_storeC, ParamsOneLayer params_loadA){
...
void generate_instruction_onelayer_B1_with_bias ( uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsOneLayer params){
    uint32_t a_iter = params.a_iter;
    uint32_t b_iter = params.b_iter;
    uint32_t k_iter = params.k_iter;
...
void generate_instruction_query_key  ( uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsOneLayer params_query,  ParamsOneLayer params_key){
    ...
    generate_instruction_onelayer_coresAB_meshesAB(inst_sequence, count4B, countInstPkt, params_query);
    ...
    a = 0; b = 0; generate_instruction_loadB(a, b, inst_sequence, count4B, countInstPkt, params_query);
    a = 0; b = 0; offset = 0; repeat_num = k_iter; 
    generate_instruction_loadA(a, b, offset, repeat_num, inst_sequence, count4B, countInstPkt, params_query);
    
    a = 1; b = 0; generate_instruction_loadB(a, b, inst_sequence, count4B, countInstPkt, params_query);
    ...
    a = 2; b = 0; generate_instruction_loadB(a, b, inst_sequence, count4B, countInstPkt, params_query);
    loadA_a = 2; loadA_b = 0; storeC_a = 1; storeC_b = 0;
    generate_instruction_K8_steady_ddr(loadA_a, loadA_b, storeC_a, storeC_b, inst_sequence, count4B, countInstPkt, params_query);

    a = 3; b = 0; generate_instruction_loadB(a, b, inst_sequence, count4B, countInstPkt, params_query);
    ...
    a = 2; b = 0; generate_instruction_loadB(a, b, inst_sequence, count4B, countInstPkt, params_key);
    loadA_a = 2; loadA_b = 0; storeC_a = 1; storeC_b = 0;
    generate_instruction_K8_steady_ddr(loadA_a, loadA_b, storeC_a, storeC_b, inst_sequence, count4B, countInstPkt, params_key);

    a = 3; b = 0; generate_instruction_loadB(a, b, inst_sequence, count4B, countInstPkt, params_key);
    loadA_a = 3; loadA_b = 0; storeC_a = 2; storeC_b = 0;
    generate_instruction_K8_steady_ddr(loadA_a, loadA_b, storeC_a, storeC_b, inst_sequence, count4B, countInstPkt, params_key);
...
void generate_instruction_query_key_value  ( uint32_t *inst_sequence, uint32_t & count4B,  uint32_t & countInstPkt, ParamsOneLayer params_query,  ParamsOneLayer params_key, ParamsOneLayer params_value){
    uint32_t a_iter = params_query.a_iter;
    uint32_t b_iter = params_query.b_iter;
    uint32_t k_iter = params_query.k_iter;
    ...
    a = 0; b = 0; generate_instruction_storeC(a, b, inst_sequence, count4B, countInstPkt, params_value);
}
