...
for (int en = 0; en < NUM_ENCODER * 2; en++){
      ...
      valid(fmap1_query, load_gold_query, 1 * INPUT_LEN * 1024);
      #endif

      #ifdef ENABLE_TEST_K
      std::cout<<"Compare key"<<std::endl;
      valid(fmap2_key, load_gold_key, 1 * INPUT_LEN * 1024);
      #endif

      #ifdef ENABLE_TEST_V
      std::cout<<"Compare value"<<std::endl;
      valid(fmap3_value, load_gold_value, 1 * INPUT_LEN * 1024);
      #endif
      ...
      valid(fmap6_norm1, load_gold_fuseddense1norm1, 1 * INPUT_LEN * 1024);
      #endif

      #ifdef ENABLE_TEST_GELU
      std::cout<<"Compare fused norm1 dense2 gelu"<<std::endl;
      valid(fmap7_dense2, load_gold_norm1dense2gelu, 1 * INPUT_LEN * 4096);
      #endif

      #ifdef ENABLE_TEST_NORM2
      std::cout<<"Compare fused dense3 norm2"<<std::endl;
      valid(fmap10_norm2, load_gold_fuseddense3norm2, 1 * INPUT_LEN * 1024);
      #endif
